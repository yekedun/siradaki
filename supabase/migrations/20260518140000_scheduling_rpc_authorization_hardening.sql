-- Harden SECURITY DEFINER scheduling/commission RPCs while preserving the
-- existing mobile owner/staff flows.
...
BEGIN
  ...
  IF TG_OP = 'UPDATE'
     ...
     AND (
       OLD.status IN ('confirmed', 'cancelled', 'completed')
       OR NEW.status IN ('confirmed', 'cancelled', 'completed')
     ) THEN
...
REVOKE EXECUTE ON FUNCTION public.create_appointment_atomic(text, uuid, uuid, uuid, timestamptz, text, text, text, uuid)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_appointment_atomic(text, uuid, uuid, uuid, timestamptz, text, text, text, uuid)
  TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.update_appointment_atomic(uuid, uuid, uuid, timestamptz, text, text, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.update_appointment_atomic(uuid, uuid, uuid, timestamptz, text, text, text)
  TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.create_block_atomic(uuid, timestamptz, timestamptz, text, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_block_atomic(uuid, timestamptz, timestamptz, text, text)
  TO service_role;

REVOKE EXECUTE ON FUNCTION public.cancel_appointment_atomic(uuid)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.cancel_appointment_atomic(uuid)
  TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.complete_appointment_with_revenue(uuid, integer)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.complete_appointment_with_revenue(uuid, integer)
  TO authenticated, service_role;
